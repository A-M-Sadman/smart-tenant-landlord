...
export default function PropertyForm() {
  ...
  const [form, setForm] = useState({ name: "", address: "", city: "", district: "", description: "" });
  ...
  useEffect(() => {
    if (isEdit && id) {
      propertyApi.get(id).then((p) =>
        setForm({ name: p.name, address: p.address, city: p.city, district: p.district, description: p.description ?? "" })
      );
    }
  }, [id, isEdit]);
      ...
      <div className="form-card">
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-actions">
            <button type="button" className="btn-secondary" onClick={() => navigate("/landlord/properties")}>
              Cancel
            </button>
